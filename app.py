...
def load_books():
    try:
        books = pd.read_csv("items_enriched_api.csv")
    except Exception as e:
    ...
    if not books.empty:
        ...
        fill_cols = [
            "title", "author", "subjects", "publisher", "isbn", "language",
            "api_description", "api_thumbnail", "api_categories",
            "api_published_date", "api_authors", "api_publisher", "api_title",
            "description", "description_x", "description_y", "categories",
            "metadata", "isbn_clean", "first_isbn"
        ]
...
def load_user_history():
    try:
        ...
        if len(cols) >= 3:
            if len(cols) > 3:
                df.columns = ["user_id", "book_id", "timestamp"] + cols[3:]
            else:
                df.columns = ["user_id", "book_id", "timestamp"]
...
def get_book_description(book_row):
    for field in ["api_description", "description", "description_x", "description_y", "metadata"]:
        if field in book_row.index:
            val = str(book_row.get(field, "")).strip()
            if val and val.lower() not in ["nan", "none", ""] and len(val) > 20:
                return val

    fallback_parts = []

    categories = str(book_row.get("api_categories", "")).strip()
    if categories and categories.lower() not in ["nan", "none", ""]:
        fallback_parts.append(f"Categories: {categories}")

    subjects = str(book_row.get("subjects", "")).strip()
    if subjects and subjects.lower() not in ["nan", "none", ""]:
        fallback_parts.append(f"Subjects: {subjects}")

    publisher = str(book_row.get("api_publisher", "")).strip()
    if not publisher or publisher.lower() in ["nan", "none", ""]:
        publisher = str(book_row.get("publisher", "")).strip()
    if publisher and publisher.lower() not in ["nan", "none", ""]:
        fallback_parts.append(f"Publisher: {publisher}")
...
def get_best_title(book_row):
    title = str(book_row.get("api_title", "")).strip()
    if not title or title.lower() == "nan":
        title = str(book_row.get("title", "")).strip()
    if not title or title.lower() == "nan":
        title = "Unknown Title"
    return title

def get_best_author(book_row):
    author = str(book_row.get("api_authors", "")).strip()
    if not author or author.lower() == "nan":
        author = str(book_row.get("author", "")).strip()
    if not author or author.lower() == "nan":
        author = "Unknown Author"
    return author

def get_best_subjects(book_row):
    subjects = str(book_row.get("api_categories", "")).strip()
    if not subjects or subjects.lower() == "nan":
        subjects = str(book_row.get("subjects", "")).strip()
    return subjects
...
def display_book_shelf(books_df, book_ids, reason="", section_title="", section_key=""):
    ...
    display_books["sort_order"] = display_books["book_id"].apply(
        lambda x: book_ids.index(x) if x in book_ids else 999999
    )
    ...
    for idx, (_, book) in enumerate(page_books.iterrows()):
        with cols[idx]:
            ...
            author = get_best_author(book)
            subjects = get_best_subjects(book)
...
def get_recommendations_for_new_user(books_df, preferences, popular_book_ids=None):
    if books_df.empty:
        return pd.DataFrame()

    scored = books_df.copy()
    scored["score"] = 0

    selected_subjects = preferences.get("subjects", [])
    selected_authors = preferences.get("authors", [])
    selected_language = preferences.get("language", "")
    reading_type = preferences.get("reading_type", "")

    # ------------------------------
    # SUBJECT / FIELD OF STUDY SCORE
    # ------------------------------
    if selected_subjects and "subjects" in scored.columns:
        for subject in selected_subjects:
            if str(subject).strip():
                scored["score"] += scored["subjects"].apply(
                    lambda x: 3 if subject.lower() in str(x).lower() else 0
                )

    # ------------------------------
    # AUTHOR SCORE
    # ------------------------------
    if selected_authors and "author" in scored.columns:
        for author in selected_authors:
            if str(author).strip():
                scored["score"] += scored["author"].apply(
                    lambda x: 4 if author.lower() in str(x).lower() else 0
                )
    ...
    language_map = {
        ...
        "German": ["german", "allemand", "de"],
        "Italian": ["italian", "italien", "it"],
        "Spanish": ["spanish", "espagnol", "es"]
    }

    if selected_language in language_map and "language" in scored.columns:
        lang_keywords = language_map[selected_language]
        scored["score"] += scored["language"].apply(
            lambda x: 2 if any(k in str(x).lower() for k in lang_keywords) else 0
        )
    ...
    # ------------------------------
    type_mapping = {
        "Fiction": ["fiction", "roman", "novel", "littérature"],
        "Non-Fiction": ["history", "science", "essay", "essai", "biography"],
        "Academic": ["textbook", "education", "study", "manual", "sciences"],
        "Comics/Manga": ["comic", "comics", "manga", "bandes dessinées"]
    }

    if reading_type in type_mapping and "subjects" in scored.columns:
        type_keywords = type_mapping[reading_type]
        scored["score"] += scored["subjects"].apply(
            lambda x: 2 if any(k in str(x).lower() for k in type_keywords) else 0
        )

    # ------------------------------
    # IF USER CHOSE NOTHING USEFUL
    # ------------------------------
    has_real_preferences = (
        any(str(s).strip() for s in selected_subjects)
        or any(str(a).strip() for a in selected_authors)
        or (selected_language not in ["", "Other"])
        or (reading_type not in ["", "Mix of everything"])
    )

    if not has_real_preferences:
        if popular_book_ids:
            return books_df[books_df["book_id"].isin(popular_book_ids)].copy()
        return books_df.head(10).copy()

    # ------------------------------
    # KEEP BEST MATCHES
    # ------------------------------
    matched = scored[scored["score"] > 0].copy()

    # If no books matched, fallback to popular books
    if matched.empty:
        if popular_book_ids:
            fallback = books_df[books_df["book_id"].isin(popular_book_ids)].copy()
            return fallback
        return books_df.head(10).copy()
...
def main():
    ...
    unil_study_fields = get_unil_study_fields()
    ...
    with st.sidebar:
        ...
        show_activity = (
            (st.session_state.user_type == "existing" and st.session_state.existing_user_submitted and st.session_state.user_id is not None)
            or (st.session_state.user_type == "new" and st.session_state.new_user_submitted)
            or (
        ...
        if show_activity:
            ...
            active_sidebar_user_id = None
            if (
                st.session_state.user_type == "existing"
                and st.session_state.existing_user_submitted
                and st.session_state.user_id is not None
            ):
                active_sidebar_user_id = st.session_state.user_id

            if active_sidebar_user_id is not None and not user_history.empty:
                user_book_ids = user_history[
                    user_history["user_id"] == active_sidebar_user_id
                ]["book_id"].tolist()
                borrowed_count = len(set(user_book_ids))

                user_books_info = books_df[books_df["book_id"].isin(user_book_ids)]
                if not user_books_info.empty:
                    log_book_options = [
                        (int(row["book_id"]), str(row.get("title", "Unknown"))[:50])
                        for _, row in user_books_info.iterrows()
                    ]
            ...
            st.markdown("---")
            st.subheader("🏆 Challenge of the Month")
            st.caption(f"{current_month}: read 180 minutes")

            progress = min(user_minutes / challenge_goal, 1.0)
            st.progress(progress)
            st.caption(f"{user_minutes} / {challenge_goal} min")

            if progress >= 1.0:
                st.success("Challenge complete!")
            elif progress >= 0.5:
                st.info(f"{challenge_goal - user_minutes} min to go")
            else:
                st.warning(f"{challenge_goal - user_minutes} min remaining")
            ...
            with st.form("log_reading_form", clear_on_submit=True):
                if log_book_options:
                    ...
                    selected_book = st.selectbox(
                        "Book",
                        options=list(book_choices.keys()),
                        key="log_book_select"
                    )
                else:
                    selected_book = None
                    st.caption("Borrow, like, or save books first.")
                ...
                log_submitted = st.form_submit_button("Add Reading Entry", use_container_width=True)
            ...
            if log_submitted and selected_book:
                ...
                st.toast(f"Logged {log_minutes} minutes")
                st.rerun()
            ...
            for rank, entry in enumerate(all_entries[:5], 1):
                ...
                st.markdown(
                    f"<div style='{style} font-size:12px; padding:2px 0;'>{medal} {entry['name']} — {entry['minutes']} min</div>",
                    unsafe_allow_html=True
                )
            ...
            if st.session_state.user_type == "existing":
                st.caption(f"Borrowed / read books: {borrowed_count}")
            else:
                st.caption("Your personal activity")

            if st.button(
                f"Saved for Later ({len(st.session_state.saved_books)})",
            ...
            if st.button(
                f"Liked Books ({len(st.session_state.liked_books)})",
                use_container_width=True,
                key="liked_page_btn"
            ):
            ...
            if st.session_state.user_type == "existing":
                st.caption(f"Already read / borrowed: {borrowed_count}")

                if st.button(
                    f"Borrowed Books ({borrowed_count})",
                    use_container_width=True,
                    key="borrowed_page_btn"
                ):
                    st.session_state.page = "borrowed_page"
                    st.rerun()

            if st.button(
                f"Not Interested ({len(st.session_state.disliked_books)})",
                use_container_width=True,
                key="hidden_page_btn"
            ):
    ...
    if st.session_state.page == "book_detail":
        ...
        publisher = str(book.get("api_publisher", "")).strip()
        if not publisher or publisher.lower() == "nan":
            publisher = str(book.get("publisher", "")).strip()

        published_date = str(book.get("api_published_date", "")).strip()
        isbn = str(book.get("isbn", "")).strip()
        lang = str(book.get("language", "")).strip()
        ...
        with col1:
            ...
            st.markdown(
                f'<div class="detail-meta-value">{publisher if publisher and publisher != "nan" else "Not available"}</div>',
                unsafe_allow_html=True
            )
            st.markdown('<div class="detail-meta-label">Published Date</div>', unsafe_allow_html=True)
            st.markdown(
                f'<div class="detail-meta-value">{published_date if published_date and published_date != "nan" else "Not available"}</div>',
                unsafe_allow_html=True
            )

        with col2:
            st.markdown('<div class="detail-meta-label">Category</div>', unsafe_allow_html=True)
            st.markdown(
                f'<div class="detail-meta-value">{categories if categories else "Not available"}</div>',
                unsafe_allow_html=True
            )
        ...
        with col3:
            ...
            st.markdown(
                f'<div class="detail-meta-value">{isbn if isbn and isbn != "nan" else "Not available"}</div>',
                unsafe_allow_html=True
            )
            st.markdown('<div class="detail-meta-label">Language</div>', unsafe_allow_html=True)
            st.markdown(
                f'<div class="detail-meta-value">{lang if lang and lang != "nan" else "Not specified"}</div>',
                unsafe_allow_html=True
            )
        ...
        if similar_ids:
            display_book_shelf(
                books_df,
                similar_ids,
                reason="Similar to this book",
                section_title="Similar Books",
                section_key="detail_similar"
            )
        return
    ...
    # ============================================================
        ...
        with col1:
            user_name = st.text_input(
                "Your name (optional)",
                value=st.session_state.last_user_name,
                placeholder="e.g. nini"
            )
        with col2:
            user_id_input = st.number_input(
                "Your Library ID (1 - 7837)",
                min_value=1,
                max_value=7837,
                value=int(st.session_state.last_user_id) if st.session_state.last_user_id is not None else 606,
                step=1
            )
        with col3:
            st.markdown("")
        ...
        if get_recs:
            if user_id_input in recommendations:
        ...
        if active_user_id is not None and active_user_id in recommendations:
            st.session_state.user_id = active_user_id

        if (
            st.session_state.existing_user_submitted
            and active_user_id is not None
            and active_user_id in recommendations
        ):
            st.session_state.user_id = active_user_id
            ...
            display_book_shelf(
                ...
                section_key="existing_rec"
            )

            st.markdown(
                '<div class="section-header">👥 Book Club — Connect With Similar Readers</div>',
                unsafe_allow_html=True
            )
            ...
            if similar_readers:
                for reader in similar_readers[:1]:
                    st.markdown(f"""
                    ...
                    if reader["new_books"]:
                        display_book_shelf(
                            books_df,
                            reader["new_books"][:5],
                            reason=f"Reader #{reader['user_id']} also enjoyed this",
                            section_title="",
                            section_key=f"bookclub_{reader['user_id']}"
                        )
            else:
            ...
            if fav_authors:
                ...
                if not author_books.empty:
                    display_book_shelf(
                        books_df,
                        author_books.head(10)["book_id"].tolist(),
                        reason="✍️ By authors you already enjoy",
                        section_title="✍️ More From Your Favorite Authors",
                        section_key="existing_authors"
                    )
            ...
            if popular_book_ids:
                display_book_shelf(
                    books_df,
                    popular_book_ids[:10],
                    reason="📈 Most borrowed in our library",
                    section_title="🇨🇭 Popular at UNIL (Most Read)",
                    section_key="existing_popular"
                )
    ...
    # ============================================================
        ...
        with st.form("new_user_form"):
            ...
            with col1:
                language = st.selectbox(
                    ...
                    ["French", "English", "German", "Italian", "Spanish", "Other"]
                )
                study_subject = st.selectbox(
                    "Field of study",
                    options=["Select your field of study"] + unil_study_fields,
                    index=0
                )
                fav_subjects_selected = st.multiselect(
                    "Favorite reading subjects",
                    options=all_subjects[:200]
                )

            with col2:
                fav_authors_selected = st.multiselect(
                    "Favorite authors",
                    options=all_authors[:500]
                )
                liked_books_selected = st.multiselect(
                    "Books you've enjoyed",
                    options=all_titles[:500]
                )
                reading_type = st.selectbox(
                    "What type of reading do you prefer?",
                    ["Fiction", "Non-Fiction", "Academic", "Comics/Manga", "Mix of everything"]
                )
        ...
        if submitted:
            ...
            selected_study_subject = ""
            if study_subject != "Select your field of study":
                selected_study_subject = study_subject
            ...
            rec_books = get_recommendations_for_new_user(
                books_df,
                preferences,
                popular_book_ids
            )
            ...
            if not rec_books.empty:
                display_book_shelf(
                    books_df,
                    rec_books["book_id"].head(10).tolist(),
                ...
                display_book_shelf(
                    books_df,
                    popular_book_ids[:10],
                    reason="Most read books at UNIL",
                    section_title="Recommended For You",
                    section_key="new_rec"
                )

            if preferences["subjects"]:
                subject_popular = books_df[
                    books_df["subjects"].apply(
                        lambda x: any(
                            s.lower() in str(x).lower() for s in preferences["subjects"][:3]
                ...
                if not subject_popular.empty:
                    display_book_shelf(
                        books_df,
                        subject_popular.sample(min(10, len(subject_popular)))["book_id"].tolist(),
                        reason="Popular in your field of interest",
                        section_title="Popular in Your Field",
                        section_key="new_field"
                    )

            if preferences["authors"]:
                author_recs = books_df[
                    books_df["author"].apply(
                        lambda x: any(
                            a.lower() in str(x).lower() for a in preferences["authors"]
                        ) if pd.notna(x) else False
                    )
                ]
                if not author_recs.empty:
                    display_book_shelf(
                        books_df,
                        author_recs.head(10)["book_id"].tolist(),
                        reason="By authors you mentioned",
                        section_title="From Authors You Like",
                        section_key="new_authors"
                    )
            ...
            if st.session_state.saved_books:
                ...
                display_book_shelf(
                    ...
                    saved_ids[:10],
                    reason="Saved for later",
                    section_title="Next Up",
                    section_key="new_saved"
                )
    ...
    # ============================================================
        ...
        if len(liked_ids) == 0:
            ...
            display_book_shelf(
                books_df,
                liked_ids,
                reason="From your liked books",
                section_title="Liked Books",
                section_key="liked_page"
            )
    ...
    # ============================================================
        ...
        if len(saved_ids) == 0:
            ...
            display_book_shelf(
                ...
                reason="Saved for later",
                section_title="Saved Books",
                section_key="saved_page"
            )

    # ============================================================
    # BORROWED PAGE
    # ============================================================
    elif st.session_state.page == "borrowed_page":
        col_back, col_title = st.columns([1, 5])

        with col_back:
            if st.button("← Back", key="back_from_borrowed"):
                st.session_state.page = "existing_user"
                st.rerun()
        ...
        if st.session_state.user_id is None or user_history.empty:
            st.info("No borrowed books found.")
        else:
            borrowed_ids = user_history[
                user_history["user_id"] == st.session_state.user_id
            ]["book_id"].tolist()

            if not borrowed_ids:
                st.info("No borrowed books found for this user.")
            else:
                seen = set()
                borrowed_ids_unique = []
                for bid in borrowed_ids:
                    if bid not in seen:
                        seen.add(bid)
                        borrowed_ids_unique.append(bid)

                display_book_shelf(
                    books_df,
                    borrowed_ids_unique,
                    reason="From your borrowing history",
                    section_title="Borrowed Books",
                    section_key="borrowed_page"
                )
    # ============================================================
    # SEARCH
    # ============================================================
    if st.session_state.page not in ["home", "book_detail"]:
        ...
        if search_query:
            results = books_df[
                books_df.apply(
                    lambda row:
                        search_query.lower() in str(row.get("title", "")).lower()
            ...
            if not results.empty:
                display_book_shelf(
                    books_df,
                    results.head(10)["book_id"].tolist(),
                    reason="Search result",
                    section_title=f"Results for '{search_query}'",
                    section_key="search"
                )
            else:
